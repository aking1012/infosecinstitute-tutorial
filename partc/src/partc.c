/*
 ===============================================================================
 Name        : partc.c
 Author      : aking1012.com@gmail.com - borrowed code noted below
 Version     :
 Copyright   : Parts of this are Mozilla licensed, so the rest has to be as well
 	 	 	 	 if you're wondering that's basically copyleft
 	 	 	 	 funny how the mozilla license is copyleft, but they get upset
 	 	 	 	 if linux devs patch something downstream before they get around
 	 	 	 	 to it upstream...hence iceweasel
...
//Begin borrowed code with credit given
/*
...
int MapRes(){
	...
	HRSRC hres = FindResource(hinst, MAKEINTRESOURCE(130), MAKEINTRESOURCE(RT_RCDATA));
	unsigned int hfileres = LoadResource(hinst, hres);
	unsigned int dwSize = SizeofResource(hinst, hres);
	unsigned char * data = (unsigned char *)malloc(dwSize);
	memcpy(data, hfileres, dwSize);
	...
	typedef void (*pfunc)();
	pfunc hello = (pfunc)MemoryGetProcAddress(module, "hello");
	hello();
	hello();
